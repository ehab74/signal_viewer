...
class Ui_MainWindow(QMainWindow):
    ...
    signalGraph = [] #checkpoints for the latest zoom/seek action on the graph's X-axis for all subwindows
    ...
    def zoomIn(self, subWindow):
        ...
        if flag and self.zoomRange[subWindowIndex-1] > 50:
            ...
            if (self.zoomRange[subWindowIndex-1] <= 50): #Disables the zoom in button when the user reaches a certain range
                self.actionZoomIn.setEnabled(False)   
            if self.zoomRange[subWindowIndex-1] < len(self.signals[subWindowIndex-1]):
                self.actionZoomOut.setEnabled(True) #Enables the zoom out button when the user reaches a certain zoom-in-range
    ...
    def read_edf(self, filename):
        ...
        sigbufs = np.zeros((n, f.getNSamples()[0])) #initiats an ndarry of zeroes
        for i in np.arange(n):
            sigbufs[i, :] = f.readSignal(i) #store the signal values in the array
